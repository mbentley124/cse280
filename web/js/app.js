...
function draw_buses(bus_obj, map) {
    ...
    $.each(bus_obj, function(k, bus) {
        ...
        if ((timings == null) || (timings.length == 0)) {
            popup_content = `${service} Bus: ${bus_id} <br>On route: ${mapped_routes.get(route_id)} <br> Previous stop: ${last_stop}`;
        } else {
            ...
            popup_content = `${service} Bus: ${bus_id} <br>On route: ${mapped_routes.get(route_id)} <br> ${last_stop} => ${next_stop} in ${time_str}`;
        }
